...
if st.session_state.role == 'admin':
    ...
    cat_count = df["Z_Category"].value_counts().reset_index()
    cat_count.columns = ["Kategori", "Jumlah"]
    fig = px.pie(cat_count, names="Kategori", values="Jumlah", hole=0.3)
    st.plotly_chart(fig, use_container_width=True)

    st.subheader("🧱 Nine Box Matrix (Z-Score)")
    fig2 = px.scatter(
        df,
        x="Z_KPI",
        y="Z_Potential",
    ...
    if st.button("Download PDF Ringkasan"):
        ...
        pdf.ln(10)
        for k in cat_count["Kategori"]:
            jumlah = cat_count.loc[cat_count["Kategori"] == k, "Jumlah"].values[0]
            text = f"{k}: {jumlah if jumlah > 0 else '-'}"
    ...
    for k in cat_count["Kategori"]:
        jumlah = cat_count.loc[cat_count["Kategori"] == k, "Jumlah"].values[0]
        st.markdown(f"- **{k}**: {jumlah if jumlah > 0 else '-'}")
