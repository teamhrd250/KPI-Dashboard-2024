...
st.caption("🗣️ Klik tombol di bawah ini untuk bicara. Pastikan browser mendukung (Google Chrome).")

# Tombol voice input pakai Web Speech API (JS)
...
if st.button("Tanya AI") and user_question:
    with st.spinner("Sedang menjawab berdasarkan data KPI..."):
        try:
            openai.api_key = "YOUR-API-KEY"  # Ganti dengan API key lo
            prompt = f"""Diberikan data karyawan berikut:
{df[['Nama', 'Nilai KPI (%)', 'Nilai Potential (%)', 'Z_Category']].to_string(index=False)}

Jawab pertanyaan ini secara profesional:
{user_question}
"""
            ...
            audio_path = "/tmp/kpi_ai_voice.mp3"
            tts.save(audio_path)
            st.audio(audio_path)
        except Exception as e:
            st.error(f"Gagal memproses AI: {e}")

# ============================ AI ASSISTANT BERBASIS SUARA ============================
st.markdown("---")
st.header("🎙️ Tanya AI Berdasarkan Dashboard KPI")
st.caption("Klik tombol di bawah ini untuk bicara. Gunakan Google Chrome.")
...
user_question = st.text_input("Pertanyaan Anda (hasil dari suara):")
...
Jawab dengan gaya profesional HR untuk pertanyaan ini:
{user_question}
"""
            try:
                response = openai.Completion.create(
                ...
                from gtts import gTTS
                tts = gTTS(answer, lang='id')
                audio_path = "/tmp/kpi_voice_answer.mp3"
                tts.save(audio_path)
                st.audio(audio_path)
            except Exception as e:
                st.error(f"Terjadi kesalahan saat menjawab: {e}")