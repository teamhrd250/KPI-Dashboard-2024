...
st.write("🔊 Klik tombol di bawah ini untuk berbicara (Gunakan browser Google Chrome).")

st.components.v1.html('''
<script>
    const startButton = document.createElement("button");
    startButton.innerText = "🎤 Mulai Bicara";
...
user_question = st.text_input("Pertanyaan Anda (dari suara):")

if st.button("Tanya AI"):
    if user_question:
        with st.spinner("Sedang menjawab..."):
            prompt = f"Diberikan data berikut:\n{df[['Nama', 'Nilai KPI (%)', 'Nilai Potential (%)', 'Z_Category']].to_string(index=False)}\n\nJawab pertanyaan ini secara profesional: {user_question}"
            ...
            audio_file = "/tmp/jawaban_kpi_ai.mp3"
            tts.save(audio_file)
            st.audio(audio_file)

# ===================== 🔁 FILTER INTERAKTIF & VISUALISASI =====================
st.markdown("---")
st.subheader("🔎 Filter Interaktif KPI Dashboard")

# PILIH KATEGORI Z_SCORE
kategori_opsi = df['Z_Category'].unique().tolist()
selected_kategori = st.selectbox("Pilih Kategori (Z_Category):", ["Semua"] + kategori_opsi)

# PILIH KARYAWAN
nama_opsi = df['Nama'].unique().tolist()
selected_nama = st.selectbox("Pilih Karyawan:", ["Semua"] + nama_opsi)

# FILTER DATA
filtered_df = df.copy()
if selected_kategori != "Semua":
    filtered_df = filtered_df[filtered_df['Z_Category'] == selected_kategori]
if selected_nama != "Semua":
    filtered_df = filtered_df[filtered_df['Nama'] == selected_nama]

# TAMPILKAN TABEL FILTERED
st.markdown("### 📋 Tabel Hasil Filter")
st.dataframe(filtered_df, use_container_width=True)

# EXPORT CSV
csv_download = filtered_df.to_csv(index=False).encode('utf-8')
st.download_button("⬇️ Download CSV", data=csv_download, file_name="filtered_kpi.csv", mime="text/csv")

# PLOT PIE Z_CATEGORY
import plotly.express as px

st.markdown("### 📊 Komposisi Z-Score (Pie Chart)")
fig_pie = px.pie(df, names="Z_Category", title="Distribusi Z-Score")
st.plotly_chart(fig_pie, use_container_width=True)

# PLOT SCATTER NINE BOX
st.markdown("### 🟧 Nine Box Scatter KPI Matrix")
fig_scatter = px.scatter(df, x='Z_KPI', y='Z_Potensi',
                         color='Z_Category',
                         hover_name='Nama',
                         title="Nine Box Grid (Z-KPI vs Z-Potensi)",
                         size_max=15)
fig_scatter.update_layout(height=500)
st.plotly_chart(fig_scatter, use_container_width=True)

# RADAR CHART PER KARYAWAN (SIMULASI SEDERHANA)
st.markdown("### 🕸️ Radar Chart KPI vs Potensi (per Karyawan)")
if selected_nama != "Semua":
    try:
        import plotly.graph_objects as go
        ...
        fig_radar = go.Figure()
        fig_radar.add_trace(go.Scatterpolar(
            r=[row['Nilai KPI (%)'], row['Nilai Potential (%)']],
            theta=["KPI", "Potensi"],
            fill='toself',
            name=selected_nama
        ))
        ...
        st.warning("Radar chart tidak dapat ditampilkan.")
else:
    st.info("Pilih karyawan untuk lihat radar chart.")