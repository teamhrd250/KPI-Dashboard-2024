...
# 1. Menambahkan Kode Login
# Data pengguna dan kata sandi
USER_CREDENTIALS = {
    "arie hr": "Starcoms@123",  # username: password
}
...
def login():
    ...
    if login_button:
        if username in USER_CREDENTIALS and USER_CREDENTIALS[username] == password:
            st.session_state.logged_in = True  # Menandakan pengguna sudah login
            st.success("Login berhasil!")
        else:
            st.error("Nama Pengguna atau Kata Sandi salah. Coba lagi.")

# 2. Mengecek Status Login Pengguna
if "logged_in" in st.session_state and st.session_state.logged_in:
    # Setelah login berhasil, tampilkan dashboard
