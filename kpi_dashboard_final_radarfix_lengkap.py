...
if role == "admin":
        ...
        # --- PILIH KATEGORI UNTUK LIHAT RINGKASAN ---
        st.markdown("### 💡 Feedback Berdasarkan Kategori")
        ...
        feedback_data = {
            ...
            "🌟 Future Star": {
                "Deskripsi": "Potensi tinggi, performa masih berkembang. Perlu bimbingan dan coaching.",
                "Pencapaian": [
                    "Antusias belajar",
        ...
        selected = st.selectbox("Pilih Kategori", list(feedback_data.keys()))
        data = feedback_data[selected]

        st.markdown("---")
        st.markdown(f"#### 🔸 Kategori: {selected}")
        st.markdown(f"##### 📝 Deskripsi
{data['Deskripsi']}")
        st.markdown("##### 🔧 Pencapaian Umum")
        for item in data["Pencapaian"]:
            st.markdown(f"- {item}")
        st.markdown("##### ✅ Tindakan yang Disarankan")
        for action in data["Tindakan"]:
