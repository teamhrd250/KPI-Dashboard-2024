...
# Simulasi data user yang terdaftar dengan role
users_db = {
    'admin': {'password': 'sikasep123', 'role': 'admin'},
    'user1': {'password': 'simanis123', 'role': 'user'},
}
...
def login():
    ...
    with col2:
        ...
        st.markdown("## Login to KPI Dashboard", unsafe_allow_html=True)
...
if role == 'admin':
    ...
    st.title("📊 Performance & Potential - HR Dashboard (Read-Only)")
    ...
    fig_custom = px.scatter(
        ...
        symbol="Z_Category",
        title="Nine Box Matrix - Z-Score Classification"
    )
    ...
    # Ringkasan penyebaran kategori
# ==== Export Summary to PDF ====
from fpdf import FPDF

if st.button("📤 Export Ringkasan ke PDF"):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", size=12)
    pdf.cell(200, 10, txt="Ringkasan Penyebaran Karyawan - Nine Box Matrix", ln=True, align="C")
    pdf.ln(10)

    for category in feedback_dict:
        count = summary.get(category, 0)
        display = f"{count} orang" if count > 0 else "-"
        desc = feedback_dict[category]
        pdf.set_font("Arial", "B", size=12)
        pdf.cell(0, 10, txt=f"{category}: {display}", ln=True)
        pdf.set_font("Arial", "", size=11)
        pdf.multi_cell(0, 10, txt=desc)
        pdf.ln(2)

    output_path = "ringkasan_ninebox_summary.pdf"
    pdf.output(output_path)

    with open(output_path, "rb") as f:
        st.download_button("⬇️ Download PDF Ringkasan", f, file_name=output_path, mime="application/pdf")

    st.markdown("### 📋 Ringkasan Penyebaran Karyawan di Nine Box:")
    summary = df['Z_Category'].value_counts()

    feedback_dict = {
        "⭐ Star Player": "Individu dengan kinerja tinggi dan potensi tinggi. Aset strategis perusahaan.",
        "🌟 Future Star": "Potensi tinggi, performa sedang. Perlu pengembangan berkelanjutan.",
        "⚠️ Risk of Loss": "Potensi tinggi namun performa rendah. Risiko kehilangan SDM berbakat.",
        "💠 Rough Diamond": "Performa tinggi, potensi belum tergali. Perlu pembinaan arah karier.",
        "🔁 Inconsistent": "Performa tidak konsisten. Butuh stabilisasi dan pemantauan.",
        "❌ Low Performer": "Performa dan potensi rendah. Evaluasi dan rencana perbaikan dibutuhkan.",
        "📉 Limited Growth": "Potensi pengembangan terbatas. Fokus pada peningkatan kinerja dasar.",
        "🔷 Core Player": "Performa dan potensi stabil. Tulang punggung tim.",
        "📌 High Performer": "Performa tinggi dengan potensi sedang. Bisa ditingkatkan lebih lanjut."
    }

    for category in feedback_dict:
        count = summary.get(category, 0)
        display = f"{count} orang" if count > 0 else "-"
        st.markdown(f"- **{category}**: {display}  
  _{feedback_dict[category]}_")
...
# Optional: Logout button (always available when logged in)
st.sidebar.markdown("---")
